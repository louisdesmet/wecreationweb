...
function getProjects() {
  return fetch('https://142.93.239.42/api/projects', {
    headers: new Headers({
...
function getBusinesses() {
  return fetch('https://142.93.239.42/api/businesses', {
    headers: new Headers({
      'Authorization': 'Bearer ' + localStorage.getItem("token"),
      'Accept': 'application/json'
    })
...
function getEvents() {
  return fetch('https://142.93.239.42/api/users/' + JSON.parse(localStorage.getItem("user")).id + '/events', {
    headers: new Headers({
      'Authorization': 'Bearer ' + localStorage.getItem("token"),
      'Accept': 'application/json'
    })
...
function getActivities() {
  return fetch('https://142.93.239.42/api/activities', {
    headers: new Headers({
      'Authorization': 'Bearer ' + localStorage.getItem("token"),
      'Accept': 'application/json'
    })
